...
Produces a container capable of acting as a client for Amazon Web Services
...
```
$ docker/aws-client/run.sh -h

     Usage: docker/aws-client/run.sh [options]
     Options:
...
Since the interaction with AWS typically requires the use of SSH communication, the private key from the logged in users running the Docker container will be copied to the containers' ```~/.ssh``` folder. You can choose to modify the default source location by providing the ```--ssh``` option with a reference to the directory  
